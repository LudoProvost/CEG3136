...
void system_user_login_event(alarm_system_t *system, user_t *user){
  ...
  system->usr_logged_in = user;
	system->usr_logged_in->state = LOGGED_IN;		//ADDED TO MAKE CODE WORK
}
...
void system_fsm_coverage_display (alarm_system_t *system){
  ...
  printf ("FSM State Coverage:\n");
	 printf ("\tUNARMED\tARMING\tARMED\tALARMED\n");
  for (i=0; i<4; i++) {
...
void system_update_state(alarm_system_t *system){
  switch (system->state){ //transitions
    case UNARMED:
			if (system->usr_logged_in->state == LOGGED_IN) {	//user login
				system->state = WAITING_TO_ARM;
				system->tick_cnt = 0;
				system->usr_logged_in->state = LOGGED_OUT;	//logs user out after checking the login
			}
			else {	//nothing, stays unarmed
				system->state = UNARMED;
			}
      break;
    case WAITING_TO_ARM: 
			...
			else if (system->tick_cnt < 500) {	//stay arming for 500 ticks
				system->state = WAITING_TO_ARM;
				system->tick_cnt++;	//increment tick_cnt
			}
			else if (system->tick_cnt >= 500) {	//tick_cnt >= 500
				system->state = ARMED;
			}
      break;
    case ARMED: 
		if (*(system->sensor_states) != 0) {	//sensor interrupt detected
				system->state = ALARMED;
				system_alarm_set(system);	//alarm ON
			} else if(system->usr_logged_in->state == LOGGED_IN){ //user logs in
				system->state = UNARMED;
				system->usr_logged_in->state = LOGGED_OUT; //logs user out after check
			} else {	//nothing, stays armed
				system->state = ARMED;
			}
      break;
    case ALARMED: 
			if (system->usr_logged_in->is_super_user == 1 && system->usr_logged_in->state == LOGGED_IN) {	//superuser login
				system->state = UNARMED;
				system_alarm_reset(system);	//alarm OFF
				system->usr_logged_in->state = LOGGED_OUT;	//logs user out after checking the login
			}
			else {
				system->state = ALARMED;
